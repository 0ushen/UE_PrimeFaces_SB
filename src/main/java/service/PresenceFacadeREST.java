...
public class PresenceFacadeREST extends AbstractFacade<Presence> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Integer id, Presence entity) {
        super.edit(entity);
    }
    ...
    public Presence find(@PathParam("id") Integer id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Presence> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
